...
export default function DevPanel({ 
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white/[0.03] border border-white/[0.08] backdrop-blur-2xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] rounded-2xl w-full max-w-sm p-5 space-y-6">
                <div className="flex justify-between items-center">
                    <h2 className="text-xl font-bold flex items-center gap-2"><Settings2 className="text-[#D4FF00]" /> Dev Panel</h2>
                    <button onClick={onClose} className="p-2 bg-white/[0.06] rounded-full text-neutral-400 hover:text-white"><X size={20} /></button>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <button 
                        ...
                        className={`w-full p-4 rounded-xl border flex items-center justify-between transition-colors ${forceFallback ? 'bg-[#D4FF00]/10 border-[#D4FF00]/30' : 'bg-white/[0.06] border-neutral-700'}`}
                    >
                        <div className="flex items-center gap-3">
